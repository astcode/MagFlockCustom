# Current Kernel + MagDS Stack – Enterprise Readiness TODOs (Before MagWS)
...
- Build `ConfigLoader` service to merge base → environment → secrets, backed by unit tests.
...
- Add enterprise-ready controls:
  - [ ] 1. Weighted replica scoring (latency/region tags) to choose optimal promotion candidates.
  - [ ] 2. CLI to register/unregister replicas at runtime (`php mag magds:replica register|unregister`).
  - [ ] 3. Fencing + session drain (terminate old primary backends, ensure WAL sync before reintegration).
  - [ ] 4. Heartbeat cache/state so multiple kernel instances share failover context.
  - [ ] 5. Lag telemetry & alerts (replica lag/latency, failover success/failure counters).
  - [ ] 6. Post-failover validation (auto smoke test + logging) and automated reintegration checklist.
- **DoD – Replica & Failover**
...
- Provide CLI support (`php mag magds:backup run|verify`, `magds:restore`) plus log evidence.
- **DoD – Backup & Restore**
  - [ ] Backup verification proof (checksums/logs) stored and documented.
  - [ ] Restore procedure scripted and tested.
  - [ ] Backup/Restore runbook published (`docs/ops/Runbook_BackupRestore.md`).
